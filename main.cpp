...
int main(int argc, char *argv[])
{
    ...
    double T = 5;
    double discount_f = 1;
    double rho_ij[6] = {0.1, 0.3, 0.5, 0.7, 0.8, 0.95};
    double K = 100;
    double F = 100;
    double sigma = 0.4;
    ...
    for (int r = 0; r < 6; r++)
    {
        ...
        std::ofstream myfile;
        std::string name = "time_control_" + std::to_string(rho_ij[r]) + "_single_core.csv";
        myfile.open (name);
        ...
        {        
            ...
            avg_payoff = start_exp(
                n_assets, T, discount_f, rho_ij[r], K, F, sigma, wi, paths, path_len
            );
